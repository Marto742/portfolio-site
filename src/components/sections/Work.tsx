...
function ProjectCard({
  ...
  return (
    <article
      className={cn(
        'group flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-surface transition duration-300 hover:-translate-y-1 hover:border-accent/40 hover:shadow-[0_24px_50px_-24px_rgba(212,168,67,0.22)]',
        featured && 'shadow-[0_0_70px_-30px_rgba(212,168,67,0.25)]',
      )}
    >
      <div
        className={cn(
          'overflow-hidden border-b border-line bg-canvas',
      ...
      >
        <img
          ...
          className={cn(
            ...
            project.slug === 'simon-says' ? 'object-center' : 'object-top',
          )}
        />
      </div>

      <div className={cn('flex flex-1 flex-col p-8', featured && 'sm:p-10')}>
          ...
          <dl className="mt-6 grid grid-cols-3 gap-4 border-y border-line py-5">
            {tp.metrics.map((metric) => (
              <div key={metric.label}>
                <dt className="font-display text-2xl text-fg">
                  {metric.value}
                </dt>
